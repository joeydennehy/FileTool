describe('Sweep Test', ()=>{
	context ("Create And Pay Multifund Grant",() => {		
		it("Created Grant and Pay Multifund Grant Test", () => {
			...
			cy.get("input[name$='grant_date']").then((date) => {
				...
				cy.get("th:contains('ID')").parent("tr").find("td").find("a").then((Id) => {
					...
					cy.get("span:contains(MultiFund Add)").click();
					cy.wait(2000);
					cy.get("input[class='ui-autocomplete-input']").eq(1).type("Deposit Fund 1");
					cy.get("li:contains(Deposit Fund 1)").click();
					cy.get("input[value=Next]").eq(1).click();
					cy.get("input[name$='::amount']").type("1000");
					cy.get("input[value='Create Grant and Link to Master Grant']").click();
					...
					cy.get("input[id=omnisearch]").type(mExternalGrantID);
					cy.get("div:contains(Grant " + mExternalGrantID + ")").click();
					cy.get("a:contains(approve payment)").eq(0).click();
					cy.get("a:contains(approve payment)").click();
					cy.get("span:contains(Grant Payment Schedule)").parent("div").parent("caption").parent("table").find("tbody").find("tr").find("td").eq(4).find("a:contains(create)").eq(0).click();
					cy.get("span:contains(Grant Payment Schedule)").parent("div").parent("caption").parent("table").find("tbody").find("tr").eq(1).find("td").eq(4).find("a:contains(create)").click();
					cy.get("span:contains(Grant Payment Schedule)").parent("div").parent("caption").parent("table").find("tbody").find("tr").find("td").eq(5).find("a:contains(pay)").click();
					cy.get("option:contains(Checking)").eq(0).then(($option) => {
						...
						cy.get("span:contains(Grant Payment Schedule)").parent("div").parent("caption").parent("table").find("tbody").find("tr").eq(1).find("td").eq(5).find("a:contains(pay)").click();
						cy.get("option:contains(Checking)").eq(0).then(($option) => {
							cy.get("select").select($option.val());
							cy.get("th:contains(Payment Date)").parent("tr").find("td").find("button").click();
							cy.get(".ui-state-highlight").click();
