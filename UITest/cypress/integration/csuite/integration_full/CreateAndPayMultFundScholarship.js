describe('Sweep Test', ()=> {
	context("Create and Pay Multi Fund Scholarship", () => {
		it("Create and Pay Multi Fund Scholarship Test", () => {
			...
			cy.get("th:contains('ID')").parent("tr").find("td").find("a").then((Id) => {
				...
				cy.wait(2000);
				cy.get("input[class='ui-autocomplete-input']").eq(1).type("Coredata Scholarship 2");
				cy.get("li:contains(Coredata Scholarship 2)").click();
				cy.get("input[value=Next]").eq(1).click();
				cy.get("input[name$='::amount']").type("1000");
				cy.get("input[value='Create Scholarship Item and Link to Master Scholarship Item']").click();

				cy.get("th:contains(Totals)").parent("tr").find("td").eq(0).find('b').should(($amount) =>  {
					expect($amount).to.contain('2,000');
				});

				cy.get("span:contains('Add Payment Date')").click();
				cy.get("select[name$='::multi_sship_item_id']").select("Coredata Scholarship");
				cy.get("input[id$='::payment_date']").type('2020-06-01');
				cy.get("input[name$='::amount']").type('1000');
				cy.get("input[value='Save']").click();

				cy.get("span:contains('Add Payment Date')").click();
				cy.get("select[name$='::multi_sship_item_id']").select("Coredata Scholarship 2");
				cy.get("input[id$='::payment_date']").type('2020-06-01');
				cy.get("input[name$='::amount']").type('1000');
				cy.get("input[value='Save']").click();
				...
				cy.get("a:contains('pay')").eq(0).click();
				cy.get("th:contains('ID')").parent("tr").find("td").find("a").then((Id) => {
					...
					cy.get("option:contains(Checking)").eq(0).then(($option) => {
						...
						cy.get("input[type='submit']").click();
						cy.wait(2000);
						cy.get("input[id=omnisearch]").type(mScholarshipID);
						cy.get("li:contains(Scholarship " + mScholarshipID + ")").click();
						...
						cy.get("th:contains('ID')").parent("tr").find("td").find("a").then((Id) => {
							...
							cy.get("option:contains(Checking)").eq(0).then(($option) => {
								...
								cy.get("th:contains(Status)").parent("tr").find("td").should(($status) =>  {
									expect($status).to.contain("closed");
								});
							});
						});
					});
